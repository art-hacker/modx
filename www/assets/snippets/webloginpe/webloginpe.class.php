...
class WebLoginPE
{
	...
	function Register($regType, $groups, $regRequired, $notify, $notifyTpl, $notifySubject)
	{
        ...
        $this->Username = $username = $email; // HACK FIX;
...
if ($username == '' || empty($username) || trim($username) == '' ) // pixelchutes
		{			
        ...
        */
        ...
        $message = str_replace(array('[+uid+]','[+pwd+]','[+ufn+]','[+sname+]','[+sname+]','[+surl+]'), array($username,$password,$fullname,$siteName,$myEmail,$siteURL), $messageTpl);
        ...
        $_SESSION['webInternalKey'] = $NewUser['internalKey'];
        $_SESSION['webValid'] = base64_encode($password);
        $_SESSION['webUser'] = base64_encode($username);
        $_SESSION['webFailedlogins'] = 0;
        $_SESSION['webUserGroupNames'] = array(); // reset user group names
        $cookieName = 'WebLoginPE';
        $cookieValue = md5($username).'|'.$username;
			...
			if ($field!='photo' || ($_FILES['photo']['name'] !== '' && !empty($_FILES['photo']['name']))) // for update db with value and blank value (except if the field is 'photo')
{
	...
	function ValidateEmail($Email)
	{
		...
		{ 
			return $this->FormatMessage($this->LanguageArray[43]);
		}
